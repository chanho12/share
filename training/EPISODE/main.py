...
from utils.data.data_utils import create_prompt_dataset, get_unsupervised_data #clear
from utils.utils import print_rank_0, to_device, save_hf_format, set_random_seed, get_all_reduce_mean, get_optimizer_grouped_parameters, save_zero_three_model, save_PEFT_model
from utils.ds_utils import get_train_ds_config # clear
from utils.model_utils import get_lora_model  #clear 
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "Choose the model which you want to learn.",
        required=True,
    )
    parser.add_argument(
...
def main():
    ...
    model, tokenizer = get_lora_model(args.model_name, args.model_name_or_path, ds_config)
    ...
    def evaluation(model, eval_dataloader):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            loss = outputs.loss #mean or sum 인지. 
            losses += loss.float()
    ...
    for epoch in range(args.num_train_epochs):
        print_rank_0(
            ...
            args.global_rank)
        wandb.alert(title = 'problem', text = 'have some fatal error!!!!!!!')
        model.train()
        mean_loss = 0
        for step, batch in enumerate(train_dataloader):
            ...
            model.backward(loss)
            model.step()
            mean_loss += loss.item() # 단순하게 sum loss 값을 누적하는 것. 
            ...
            if total_steps > 0 and total_steps % args.save_interval == 0 and args.output_dir is not None:
                ...
                wandb.alert(title = 'save', text = 'model save')
